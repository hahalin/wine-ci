<?php
  ...
  //https://github.com/ccschmitz/codeIgniter-base-model/blob/master/lib/MY_Model.php
	...
	function _validate_options_exist($options)
	{
		...
		if ($this->validate_field_existence == TRUE)
		{
			foreach ($options as $key => $value)
			{
				...
				{
					//print('You are trying to insert data into a field that does not exist.  The field "'. $field .'" does not exist in the "'. $this->primary_table .'" table.'); 
