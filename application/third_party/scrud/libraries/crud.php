...
class Crud {
    ...
    public function exportcsvall(){
    	...
    	!empty($crudAuth) &&
    	in_array($this->conf['table'] . '.created_by', $this->fields) ){
    		$conditions =  $this->conf['table'] . '.created_by = '.$crudAuth['id'].' ' ;
    	}
    ...
    private function update() {
        ...
        foreach ($this->form as $field => $v) {
            ...
            switch ($elements[0]) {
                case 'image':
					...
					$this->data[$tmpfields[0]][$tmpfields[1]]=$this->data[$field];
                    break;
				default:
				   break;
			}
        ...
        foreach ($this->data[$this->conf['table']] as $k => $v){
		    ...
		    if (isset($this->conf['form_elements'][$this->conf['table'].'.'.$k]) && 
        		$this->conf['form_elements'][$this->conf['table'].'.'.$k]['element'][0] == 'date'){
        	...
        	if (isset($this->conf['form_elements'][$this->conf['table'].'.'.$k]) &&
        	$this->conf['form_elements'][$this->conf['table'].'.'.$k]['element'][0] == 'datetime'){
        		if (__DATE_FORMAT__ == 'dd/MM/yyyy'){
        			$v = str_replace('/','-',$v);
        		}
        		$v = str2mysqltime($v,'Y-m-d H:i:s');
