...
class Auth extends CI_Controller {
	...
	function register()
	{
		...
		$this->data['upload_error_message']=array();
		...
		if ($this->form_validation->run() == true)
		{
			...
			if (!empty($_FILES['avatar']['name']))
			{
				...
				{
					$this->data['upload_error_message']=$this->image_lib->error_msg;
				}
				else 
				{
					$avatar_filename=$this->image_lib->full_dst_path;
				}
				//upload avatar pic end
			}
		...
		if (count($this->data['upload_error_message'])>0)
		{
			//if (!$this->data['message'])
			{
				//$this->data['message']=array();
			}
			//array_merge($this->data['message'],$this->data['upload_error_message']);
			$this->data['message']='';
			foreach($this->data['upload_error_message'] as $err)
			{
			...
			$this->_render_page('developer-admin/register', $this->data);	
		...
		{	
			...
			//if ($this->$image_lib)
			{
				//if (count($this->image_lib->error_msg)>0)
				{
					//$this->data['message'][]=$this->image_lib->error_msg;
				}
			}
			//echo $this->form_validation->set_value('nickname');
			$this->prepare_form_data();
			
			$this->_render_page('developer-admin/register', $this->data);	
		//$this->load->view('developer-admin/register');
		}
	}
