...
foreach ($this->colsCustom as $k => $v) {
	if ($k == 'action') {
		...
		array_unshift($matches[0], '<a class="btn btn-mini btn-danger" id="rmc_{crud_components.id}" >Remove configure</a>');
		array_unshift($matches[0], '<a class="btn btn-mini btn-info" href="'.base_url().'index.php/admin/scrud/config?com_id={crud_components.id}" >Build</a>');
		$clsWidth += 130;
...
<div style="height: 52px;">
    <div data-spy="affix" data-offset-top="90" style="
        ...
        <div class="container" style="border-bottom: 1px solid #CCC; padding-bottom:5px;padding-top:5px;
            ...
            <div style="text-align:right;width:100%;">
                <div class="btn-group" >
                <a class="btn btn-default" onclick="searchModal();">
                	<i class="glyphicon glyphicon-search"></i> 
                	<?php echo $lang->line('search');?>
                </a>
                <?php
		        $q = $this->queryString;
		        if (isset($q['xtype']))
		            unset($q['xtype']);
		        ?>
                <a class="btn btn-default" href="?<?php echo http_build_query($q, '', '&'); ?>">
	  ...
	  <div class="modal-dialog" role="document" >
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        <div id="scrud_model_search"><center><img src="<?php echo __MEDIA_PATH__; ?>icons/loading.gif" /></center></div>
	      </div>
    ...
    <?php
     foreach ($this->results as $result) { 
    	$rs1 = null;
    	if (file_exists(__DATABASE_CONFIG_PATH__ . '/' . $this->da->database. '/'.sha1('com_'.$result['crud_components']['id']) . '/' . $result['crud_components']['component_table'] . '.php')) {
    		$rs1 = str_replace("<?php exit; ?>\n", "", file_get_contents(__DATABASE_CONFIG_PATH__ . '/' . $this->da->database. '/'.sha1('com_'.$result['crud_components']['id']) . '/' . $result['crud_components']['component_table'] . '.php'));
    	}
    	if (!empty($rs1)){
    ?>
